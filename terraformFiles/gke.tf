resource "google_container_cluster" "my_cluster" {
  ...
  remove_default_node_pool = true  #to remove the default node pool
  initial_node_count       = 1
  networking_mode= "VPC_NATIVE" #enables IP aliasing then use ip allocation policy

    ip_allocation_policy { #ranges of cidrs
        cluster_ipv4_cidr_block = "192.168.0.0/21"
        services_ipv4_cidr_block= "192.168.8.0/27"
    }
  network_policy {
    enabled= false  #block all 
  }
  private_cluster_config {
    ...
    master_authorized_networks_config { #make the management_subnet is one who can access to the cluster 
      cidr_blocks {
